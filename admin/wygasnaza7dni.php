...
echo '<b>Og³oszenia które wygasaja w przeciagu tygodnia:</b><br><br>';
$zapytanie = 'select * from '.PREFIX.'firmy WHERE (wygasa-604800)<"'.time().'" AND wygasa>"'.time().'" ORDER BY wygasa ASC';
$wykonaj = mysql_query($zapytanie);

while($w = mysql_fetch_array($wykonaj))
{

$dni = round(($w['wygasa']-time())/86400);


echo '<h1>'.$w['nazwa_firmy'].'</h1>
      <h2>Wygasa za '.$dni.' dni (dok³adnie: '.date("Y-m-d H:i:s",$w['wygasa']).')</h2>
...
echo '</div><br style="clear: both"><form action="index.php?admin=za7dni" method="post">
